...
class GitHubPRCreator {
  ...
  generateCommitMessage(analysisResults, fixResults) {
    ...
    return `🤖 AI-powered fix: Resolved ${fixCount}/${failureCount} test failures
...
- Generated by AI Agent on ${new Date().toISOString()}`;
  ...
  generatePRDetails(analysisResults, fixResults) {
    const title = `🤖 AI Fix: Resolve ${fixResults.successfulFixes} test failure(s)`;
    
    let body = `## 🤖 Automated Fix by AI Agent
...
- **Fix Applied:** ${result.suggestedFix ? '✅' : '❌'}
...
`;
    ...
    if (fixResults.appliedFixes) {
      for (const fix of fixResults.appliedFixes) {
        body += `- ${fix.success ? '✅' : '❌'} ${fix.test} (${fix.file})
`;
      }
    }
...
`;
  ...
  async addReviewers(prNumber, reviewers) {
    ...
    try {
      ...
      console.error(`⚠️  Failed to add reviewers: ${error.message}`);
    }
  }
}
